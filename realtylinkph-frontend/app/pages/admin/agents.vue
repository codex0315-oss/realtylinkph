      ...
      <div v-for="agent in agents" :key="agent.id" class="card p-5">
        <div class="flex items-start gap-4 flex-wrap">
          ...
          <div class="flex gap-2 flex-wrap">
            <!-- The real check. The AI is advisory; this is where the admin
                 confirms the number against PRC's own records. -->
            <a
              v-if="agent.verification_links?.[0]"
              :href="agent.verification_links[0].url"
              target="_blank"
              rel="noopener"
              class="inline-flex items-center gap-1.5 text-xs font-semibold text-brand-navy border border-brand-navy/20 rounded-lg px-3 py-1.5 hover:border-brand-gold hover:text-brand-gold transition-colors"
              :title="agent.verification_links[0].note"
            >
          ...
          <div v-if="agent.verification_links?.length" class="mt-3">
            ...
            <ul class="space-y-1">
              <li v-for="l in agent.verification_links" :key="l.url" class="text-xs leading-snug">
                ...
                <span class="text-gray-400 dark:text-white/40"> — {{ l.note }}</span>
              </li>
            </ul>
          </div>
        </div>
